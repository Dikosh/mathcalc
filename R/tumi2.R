...
tumi2 <- function(x){
  ...
  #qplot(TOTAL_LIQUIDITY_DATAMART_1$Count,TOTAL_LIQUIDITY_DATAMART_1$M_SALES_DAYS)
  ...
  summary(range01(data$Sales_days_group_pred)*20-10)
  data$Sales_days_group_pred_coef <- range01(data$Sales_days_group_pred)*20-10
  ...
  data$flag2 <- NULL
  data$flag1
  data$SCORING <- data$Liquidity + data$Sales_days_group_pred_coef + data$ADVERT_ORDER + data$PREDICTABLE + data$ACTIVITY
